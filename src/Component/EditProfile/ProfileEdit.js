import React from 'react';
import '../EditProfile/ProfileEdit.css';
import { IoCloudUploadOutline } from "react-icons/io5";
import { useNavigate } from "react-router-dom";

const ProfileEdit = () => {

    const navigate = useNavigate();

    const handleshowprofile = () => {
        navigate("/ProfileComponents");
    };
    
    return (
        <div>
            <div className="bb">
                <div className="row">
                    <div className="col">
                        <div className="profileBackground background-image">
                            <button className="EC">
                                <span> Upload Photo <IoCloudUploadOutline /></span>
                                <img
                                    className="card-img-top big-profile-photo"
                                    alt=""
                                    style={{ width: "100%", maxHeight: "400px" }}
                                />
                            </button>
                        </div>
                        <div className="d-sm-flex align-items-start text-center text-sm-start">
                            <div className="profileAvatar avatar-xxl mt-n5 mb-3" style={{ width: "210px", height: "190px", marginTop: "-60px", marginLeft: "60px" }} >
                                <button className="rounded-circle mb-5 " style={{ width: "100%", height: "100%", border: "none" }}>
                                    <img
                                        className="rounded-circle mb-5 "
                                        style={{ width: "100%", height: "100%" }}
                                        src="data:image/png;base64,"
                                        alt="profile"
                                    />
                                </button>
                            </div>
                            <div className="ms-sm-4 mt-sm-3" style={{ width: "200px" }}>
                                <h1 className="ProfileName mb-0 h4">
                                    User Name  <i className="bi bi-patch-check-fill text-success small" />
                                </h1>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="bio1">
                    <input type="text" className="bio2" placeholder="edit your bio..." required />
                </div>
                <div className="form-group field">
                    <input type="text" className="form-field-2" placeholder="edit your name..." name="name" id="name" required />
                </div>
                <div className="But">
                        <div class="flex-grid-center" style={{marginRight:"600px"}}>
                            <button class="pure-button fuller-button blue" onClick={handleshowprofile}>save</button>
                            <button class="pure-button fuller-button red" onClick={handleshowprofile}>cancel</button>
                        </div>
                    </div>
            </div>

        </div>
    )
}

export default ProfileEdit